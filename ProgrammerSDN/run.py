...
def file_attch_step(message):
    ...
    if userid == 1625235944:
        AddNewCommand(message,newCommandProg[userid]["lan"],newCommandProg[userid]["id"],newCommandProg[userid]["input"],newCommandProg[userid]["example"],newCommandProg[userid]["file_id"],newCommandProg[userid]["lang"])
    else:
        AddNewCommandUser(message,newCommandProg[userid]["lan"],newCommandProg[userid]["id"],newCommandProg[userid]["input"],newCommandProg[userid]["example"],newCommandProg[userid]["file_id"],newCommandProg[userid]["lang"])
...
def command_id_step(message):
    ...
    if userLang == "en":
        msg = bot.reply_to(message,"Enter User Input\nWhen User Enter This It'll Get The Example")
        bot.register_next_step_handler(msg, lang_step2)
    else:
        msg = bot.reply_to(message,"الان ادخل الامر الذي سيكتبه المستخدم للحصول علي المثال \nمثال\nحلقة التكرار بلغة بايثون")
        bot.register_next_step_handler(msg, lang_step2)
...
def lan_step(message):
    ...
    for x in emptyExamples:
        markup.add(str(x))
    if userLang == "en":
        msg = bot.reply_to(message,"Now Enter Command ID For Example:- \nfor loop python",reply_markup=markup)
        bot.register_next_step_handler(msg, command_id_step)
    else:
        msg = bot.reply_to(message,"الان قم بأدخال معرف الأمر :- علي سبيل المثال \nfor loop python",reply_markup=markup)
        bot.register_next_step_handler(msg, command_id_step)
...
def AddNewCommandUser(message,commandLanguage,command,commandInput,commandExample,file_id="",lang="en"):
    f = open("usersPost.json","r")
    txtJson = json.loads(f.read())
    f.close()
    ...
    bot.send_poll("-1001604626385","is that's right ?\n id : #"+str(msg.id),["yes","no"],reply_to_message_id=msg.id)
    try:
        txtJson["users"][message.from_user.id]["Pending"].append({"username":message.from_user.username,"pid":msg.id,"id":command,"input":commandInput,"lan":commandLanguage,"example":commandExample,"file_id":file_id,"lang":lang})
    except:
...
def AddNewCommand(message,commandLanguage,command,commandInput,commandExample,file_id="",lang="en"):
    f = open("commands.json","r")
    txt = str(f.read())
    txtJson = json.loads(txt)
    f.close()
...
def insert_keyword_isRegex(message,newKeyword):
    isRegex = message.text
    if isRegex == "YES":
        newKeyword._isRegex = True
    elif isRegex == "NO":
        newKeyword._isRegex = False
    else:
        return bot.reply_to(message,"Not An Option")
    lan = newKeyword._language
    txtJson = checkLanguage(lan)
    markup = types.ReplyKeyboardMarkup(row_width=2)
    try:
        for x in range(len(txtJson["Languages"][lan]["Keywords"])):
            keyword = txtJson["Languages"][lan]["Keywords"][x]["keyword"]
            keywordColor = txtJson["Languages"][lan]["Keywords"][x]["color"]
            markup.add(keywordColor+" "+keyword)
    except:
        something=1
...
def insert_keyword_isBold(message,newKeyword):
    isBold = message.text
    if isBold == "YES":
        newKeyword._isBold = True
    elif isBold == "NO":
        newKeyword._isBold = False
    else:
        return bot.reply_to(message,"Not An Option")
    msg = bot.reply_to(message,"Is Regex")
    bot.register_next_step_handler(msg,insert_keyword_isRegex,newKeyword)
def insert_keyword(message,newKeyword):
    ...
    markup = types.ReplyKeyboardMarkup(row_width=2)
    markup.add("YES", "NO")
    msg = bot.reply_to(message,"Is Bold ?",reply_markup=markup)
    bot.register_next_step_handler(msg,insert_keyword_isBold,newKeyword)
...
def replyMarkupLanguages():
    ...
    return markup
def preview_page(message,html,saveAs):
    from selenium import webdriver
    f = open(saveAs,"w")
    f.write(html)
    f.close()
...
def gen_code_code(message,lan):
    ...
    try:
        for lineContent in code.splitlines():
            ...
            styledCode += lineContent+"\n"
    except:
        txtJson["Languages"][lan]["Keywords"] = []
    ...
    styledCode = styledCode.replace(")",html_style.setColor(")","grey"))
    quiz_html_1 = quiz_html.split('<code><pre>')[0]+'<code><pre>'+styledCode+"\n"
    quiz_html_2 = quiz_html.split('<code><pre>')[1]
    quiz_html_2 = quiz_html_2.replace('<h2 id="language" style="position:fixed;bottom:10px;left:43%;text-align:center;color:green;font-size:25px">','<h2 id="language" style="position:fixed;bottom:10px;left:43%;text-align:center;color:green;font-size:25px">'+lan)
    ...
    f.write(str(toWrite))
    f.close()
    preview_page(message,toWrite,path)
