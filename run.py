...
def test(message):
    ...
    if message_text == "getInfo":
        ...
        for line in p.splitlines():
            ...
            bot.reply_to(message,reply)
    if message_text == "all hosts count":
        getHostsCount(message)
    if message_text == "get hosts txt":
        getHostsTxt(message)
        ...
        if theCommand == "header":
            if str(message_text.split(" ")[0]) == "get" and str(message_text.split(" ")[1]) == "hosts":
                ...
                for text in splitted_text:
                    bot.reply_to(message, text)
                return
    if message_text == "bug" or message_text == "bugs":
